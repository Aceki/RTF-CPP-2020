...
{
	return Vector3D::i() * (vec1.m_y * vec1.m_z - vec2.m_y * vec2.m_z) +
		   Vector3D::j() * (vec1.m_x * vec2.m_z - vec2.m_x * vec1.m_y) +
		   Vector3D::k() * (vec1.m_x * vec2.m_y - vec2.m_x * vec1.m_y);
}

inline double Math::Vector3D::getLength() const 
{ 
...
inline Math::Vector3D Math::Vector3D::operator+(const Vector3D& vec) const
{
...
inline Math::Vector3D Math::Vector3D::operator/(const double& number) const
{
